...
export const placeFinavasiaOrder = async (
  ...
  try {
    ...
    const jData = {
     ...
     logSuccess(req, { msg: "Shoonya PlaceOrder playload ", jData });

    const body = `jData=${JSON.stringify(jData)}&jKey=${susertoken}`;

      logSuccess(req, { msg: "Shoonya PlaceOrder body ", body });
    ...
    logSuccess(req, { msg: "Shoonya PlaceOrder orderid update  ", orderid });
    ...
    try {
      ...
      logSuccess(req, { msg: "Shoonya PlaceOrder orderid fetchOrderDetailsWithRetry  ", orderDetails });

      if (orderDetails) {

        const statusNorm = normalizeStatus(orderDetails?.status);

         logSuccess(req, { msg: "Shoonya PlaceOrder orderid normalizeStatus  ", statusNorm ,orderDetails});
    ...
    await newOrder.update({
     ...
     logSuccess(req, { msg: "Shoonya  uniqueorderid updated "});
...
//  ===================== find function used in trade order ===================

export const placeFinavasiaOrder121 = async (
  user,
  reqInput,
  req,
  isLocalDbFlow = true
