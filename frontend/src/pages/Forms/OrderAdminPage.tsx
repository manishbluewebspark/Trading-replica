...
// // Build ONE big searchable string per instrument
...
//   // Base fields
//   parts.push(normSymbol, normName, normExpiry);

//   // Strike variations
//   if (normStrikeRaw) parts.push(normStrikeRaw); // "26300"
//   if (scaledStrike) {
...
//       // nifty263pe / from name
//       if (name) {
//         parts.push(normalize(name + scaledStrike + optionSide));
//       }
//       // symbol-based combos (symbol already contains it but just in case)
...
// // Filter list using the precomputed searchKey
...
//   const q = normalize(query);
//   if (!q) return [];
//   return list.filter((item) => item.searchKey && item.searchKey.includes(q));
// };
...
//         setLoading(true);
//         const filtered = filterInstruments(allInstruments, query);
//         // show all matches, or limit if you want
...
//           placeholder="Search: NIFTY02DEC2526300PE, 26300, 26300pe, nifty26300pe, 02dec2025 ..."
//           className="border p-2 w-full rounded"
//         />
//         {searchResults.length > 0 && (
//           <div className="absolute top-12 left-0 right-0 bg-white border rounded shadow-lg max-h-72 overflow-y-auto z-50 text-sm">
