...
export default function UserAddressCard() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setUserInfo({
        ...
        cityOrState: parsedUser.cityOrState || "",
  ...
  const handleSave = async () => {
    try {
      ...
      formData.append("cityOrState", userInfo.cityOrState);
      formData.append("postalCode", userInfo.postalCode);

      const response = await axios.put(`${apiUrl}/auth/address/update`, formData, {
        headers: {
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
              <div>
                ...
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{userInfo.country}</p>
              </div>

              <div>
                <p className="mb-2 text-xs text-gray-500 dark:text-gray-400">City/State</p>
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{userInfo.cityOrState}</p>
              </div>

              <div>
                <p className="mb-2 text-xs text-gray-500 dark:text-gray-400">Postal Code</p>
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{userInfo.postalCode}</p>
              </div>
            </div>
          </div>
